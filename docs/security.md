...
- Capture Mode, EXPLAIN, and Redis inspect require `APP_DEBUG=true`
- mail HTML preview stays available in monitor mode (sandboxed iframe + CSP)
...
- keep `APP_DEBUG=false` in production so EXPLAIN, Capture, and Redis inspect stay off
